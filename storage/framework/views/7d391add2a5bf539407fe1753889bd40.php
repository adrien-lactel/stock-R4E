...
(function() {
  ...
  window.lookupRomId = async function() {
    ...
    try {
      ...
      const url = `<?php echo e(url('/admin/ajax/lookup-rom-id')); ?>/${encodeURIComponent(normalizedRomId)}`;
      console.log('📡 URL:', url);
  ...
  aiAnalyzeBtnTop.addEventListener('click', async () => {
    ...
    try {
      ...
      const response = await fetch('<?php echo e(route('admin.articles.analyze-image')); ?>', {
        method: 'POST',
        headers: {
          'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
          'Content-Type': 'application/json'
            ...
            if (sugg.type_to_create && sugg.sub_category) {
              ...
              if (subCategoryId) {
                try {
                  ...
                  const response = await fetch('<?php echo e(url('/admin/taxonomy/type/auto-create')); ?>', {
                    method: 'POST',
                    headers: {
                      'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
                      'Content-Type': 'application/json',
