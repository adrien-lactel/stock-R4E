                            ...
                            <div class="flex flex-col items-center justify-between shrink-0 order-1">
                                ...
                                @if($prixR4E)
                                    <div class="w-full mt-3 bg-gradient-to-r from-green-500 to-emerald-600 rounded-lg p-3 text-center">
                                        <div class="text-xs text-white/80 uppercase font-medium mb-1">Prix R4E</div>
...
<script>
    ...
    window.openTaxonomyImageEditorModal = function() {
        ...
        if (isTaxonomyCategory) {
            ...
            knownImages = {
                ...
                display1: @json($selectedType->console_display1_url ?? null),
                display2: @json($selectedType->console_display2_url ?? null),
                display3: @json($selectedType->console_display3_url ?? null)
            };
        } else {
    ...
    @endphp
    const knownTaxonomyImages = @json($isTaxonomyCategory) ? {
        logo: @json($selectedType->logo_url ?? null),
        display1: @json($selectedType->console_display1_url ?? null),
        display2: @json($selectedType->console_display2_url ?? null),
        display3: @json($selectedType->console_display3_url ?? null)
    ...
    function renderTaxonomyImagesGrid(gridContainer, images, identifier, folder) {
        ...
        images.forEach(image => {
            ...
            @endphp
            const isTaxonomyCat = @json($isTaxonomyCat);
            if (isTaxonomyCat) {
                select.innerHTML = `
