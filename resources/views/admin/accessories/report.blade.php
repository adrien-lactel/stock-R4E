...
<div class="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
    ...
    <div class="grid grid-cols-1 md:grid-cols-5 gap-4 mb-6">
        <div class="bg-white rounded-lg shadow p-4">
            ...
            <div class="text-2xl font-bold text-indigo-600">{{ $stats['total_items'] }}</div>
            <div class="text-xs text-gray-400 mt-1">Différents types</div>
        </div>
        
        <div class="bg-white rounded-lg shadow p-4">
            <div class="text-sm text-gray-500">Quantité totale</div>
            <div class="text-2xl font-bold text-blue-600">{{ number_format($stats['total_quantity'], 0, ',', ' ') }}</div>
                ...
                @forelse($accessories as $accessory)
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4 text-sm text-gray-500">
                            {{ Str::limit($accessory->description ?? '—', 50) }}
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-center">
                            <span class="font-semibold 
                                @if($accessory->quantity <= 0) text-red-600
                                @elseif($accessory->quantity <= 5) text-orange-600
                                @else text-gray-900
                                @endif">
                                {{ number_format($accessory->quantity, 0, ',', ' ') }}
                            </span>
                        </td>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-center">
                            <form action="{{ route('admin.accessories.update-valorisation', $accessory) }}" method="POST" class="flex items-center justify-center gap-2">
                                @csrf
                                @method('PATCH')
                                <input type="number" 
                                       name="valorisation" 
                                       step="0.01" 
                                       min="0" 
                                       value="{{ $accessory->valorisation ?? 0 }}" 
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium text-gray-900">
                            {{ number_format($accessory->quantity * $accessory->purchase_price, 2, ',', ' ') }} €
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-center">
    ...
    <div class="mt-6 flex gap-4">
        <a href="{{ route('admin.accessories.index') }}" class="px-4 py-2 bg-indigo-600 text-white rounded hover:bg-indigo-700">
            📋 Gérer les accessoires
        </a>
        <a href="{{ route('admin.accessories.create') }}" class="px-4 py-2 border rounded hover:bg-gray-50">
            ➕ Créer un accessoire
        </a>
    </div>
