...
async function fetchGameSuggestions(query) {
  ...
  try {
    const url = `<?php echo e(url('admin/ajax/search-game')); ?>?platform=${platform}&query=${encodeURIComponent(query)}`;
    console.log('URL:', url);
    const response = await fetch(url);
...
(function() {
  ...
  let uploadedGameImages = <?php echo json_encode($console->article_images ?? [], 15, 512) ?>;
  let primaryImageUrl = <?php echo json_encode($console->primary_image_url ?? null, 15, 512) ?>;
