...
window.openTaxonomyImagesForArticle = function() {
  ...
  const useNameMatching = nameBasedPlatforms.some(p => subCatLower.includes(p));
  
  let identifier;
  
  if (useNameMatching) {
    // Pour WonderSwan, GameGear, MegaDrive, Saturn: utiliser le NOM
    identifier = articleTypeName;
    console.log('📝 Plateforme basée sur NOM détectée:', subCategoryName, '→ identifier:', identifier);
  } else {
    // Pour Game Boy, SNES, NES, N64: utiliser le ROM ID
    identifier = romId;
    ...
    console.log('🔢 Plateforme basée sur ROM ID détectée:', subCategoryName, '→ identifier:', identifier);
  }
  
  // Fallback sur le nom complet si aucun identifier trouvé
  if (!identifier) {
    identifier = articleTypeName;
  }
...
async function loadTaxonomyImages(identifier, folder) {
  ...
  try {
    ...
    if (data.success && data.images.length > 0) {
      ...
      // Détecter automatiquement le type (console vs jeu) en regardant les premiers types d'images
      const hasDisplayTypes = data.images.some(img => ['display1', 'display2', 'display3'].includes(img.type));
      const isConsoleCat = hasDisplayTypes;
      ...
      data.images.forEach(image => {
        ...
        // Adapter les options selon le type détecté
        if (isConsoleCat) {
          select.innerHTML = `
        ...
        // Stocker l'ancien type pour éviter les événements en boucle
        select.dataset.currentType = image.full_type;
        select.addEventListener('change', function(e) {
          const oldType = this.dataset.currentType;
          const newType = this.value;
          if (oldType !== newType) {
            renameTaxonomyImage(identifier, folder, oldType, newType);
            // Mettre à jour après confirmation
...
window.refreshGameImages = function(game, platform, identifier, folder) {
  ...
  const timestamp = Date.now();
  // Toujours utiliser R2 pour la grille de prévisualisation après fermeture du modal
  // pour garantir que les images uploadées sont bien visibles
  const r2Url = 'https://pub-ab739e57f0754a92b660c450ab8b019e.r2.dev';
  const baseUrl = r2Url + '/taxonomy';
...
// Variable pour éviter les renommages simultanés
...
async function renameTaxonomyImage(identifier, folder, oldType, newType) {
  ...
  // Empêcher les renommages simultanés
  if (isRenamingInProgress) {
    console.warn('⚠️ Renommage déjà en cours, opération ignorée');
    return;
  }
  ...
  try {
    ...
    if (data.success) {
      ...
      // Ne PAS recharger toutes les images pour éviter les événements en boucle
      // Le select affiche déjà la bonne valeur après le changement de l'utilisateur
...
window.displayGameResult = async function(game, platform) {
  ...
  editSection.appendChild(editGentifiant et le dossier AVANT de créer les boutons
  const nameBasedPlatforms = ['wonderswan', 'megadrive', 'segasaturn', 'gamegear'];
  let identifier;
  ...
  // Toujours utiliser R2 pour les images de taxonomie (source de vérité)
  const r2Url = 'https://pub-ab739e57f0754a92b660c450ab8b019e.r2.dev';
  const baseUrl = r2Url + '/taxonomy';
...
(function() {
  ...
  function addImagePreview(url, typeId, isExisting) {
    ...
    div.innerHTML = `
      ...
      <button type="button" 
              ...
              data-delete-taxonomy-image="${finalUrl}"
              data-type-id="${typeId}">
        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
        </svg>
    ...
    const deleteBtn = div.querySelector('[data-delete-taxonomy-image]');
    if (deleteBtn) {
      deleteBtn.addEventListener('click', (e) => {
        e.stopPropagation();
        deleteImage(deleteBtn.dataset.deleteTaxonomyImage, deleteBtn.dataset.typeId, div);
      });
    }
  }
  ...
  window.addArticleImageCard = function(imageSrc, fileName, status = 'uploaded', isGeneric = false) {
    ...
    card.innerHTML = `
      <div class="relative group">
        <img src="${imageSrc}" class="w-full aspect-square object-cover rounded cursor-pointer hover:opacity-90" 
             data-lightbox-url="${imageSrc}" data-is-primary="${isPrimary}">
        ...
        <div class="absolute top-2 right-2 flex gap-1">
          ...
          ` : `
            <button type="button" data-set-primary="${imageSrc}" 
            ...
            ${isGeneric ? `
              <button type="button" data-deselect-generic="${imageSrc}" 
            ...
            ` : `
              <button type="button" data-delete-image="${imageSrc}" 
    ...
    // Ajouter les event listeners au lieu d'onclick inline
    ...
    if (setPrimaryBtn) {
      setPrimaryBtn.addEventListener('click', () => {
        setPrimaryImage(setPrimaryBtn.dataset.setPrimary, setPrimaryBtn);
      });
    }
    
    const deselectBtn = card.querySelector('[data-deselect-generic]');
    if (deselectBtn) {
      deselectBtn.addEventListener('click', () => {
        deselectGenericImage(deselectBtn.dataset.deselectGeneric, deselectBtn);
      });
    }
    ...
    if (deleteBtn) {
      deleteBtn.addEventListener('click', () => {
        deleteArticleImage(deleteBtn.dataset.deleteImage, deleteBtn);
      });
    }
    
    const captionInput = card.querySelector('input[data-image-url]');
    if (captionInput) {
      captionInput.addEventListener('change', () => {
        updateArticleImageCaption(captionInput.dataset.imageUrl, captionInput.value);
      });
    }
  ...
  function updateArticleImageCard(fileName, finalUrl) {
    ...
    if (card) {
      ...
      if (badge) {
        const deleteBtn = document.createElement('button');
        deleteBtn.type = 'button';
        deleteBtn.className = 'bg-red-500 text-white p-1 rounded-full opacity-0 group-hover:opacity-100 transition-opacity hover:bg-red-600';
        deleteBtn.innerHTML = `
          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
          </svg>
        `;
        deleteBtn.addEventListener('click', () => {
          deleteArticleImage(finalUrl, deleteBtn);
        });
        badge.replaceWith(deleteBtn);
      }
